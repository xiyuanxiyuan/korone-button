<template>
  ...
  >
    ...
    <v-btn v-if="$vuetify.breakpoint.mdAndUp" class="ml-4 text-capitalize" color="red"
           @click="See('https://www.youtube.com/channel/UChAnqc_AY5_I3Px5dig3X1Q?sub_confirmation=1')">
      <v-icon left>mdi-youtube</v-icon>
      Korone Ch. 戌神ころね
    </v-btn>
    <v-btn v-if="$vuetify.breakpoint.mdAndUp" class="ml-4 text-lowercase" color="blue"
           @click="See('https://twitter.com/inugamikorone')">
      <v-icon left>mdi-twitter</v-icon>
      @inugamikorone
    </v-btn>
    <v-spacer></v-spacer>
    <v-btn v-if="$vuetify.breakpoint.mdAndUp" @click="See('https://github.com/oruyanke/korone-button')">
